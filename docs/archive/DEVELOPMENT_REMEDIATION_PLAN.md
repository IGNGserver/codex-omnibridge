# 【已归档】Codex MultiProvider 后续整改与实施路线

> ## ⚠️ 本文档已归档，禁止作为实现依据
>
> **状态：HISTORICAL ONLY。** 本文描述的是 2026-09-09 之前基于 **patched Codex Core**
> 的架构方案，该方案已被 [`docs/CCSWITCH_MECHANISM_AUDIT_AND_REBUILD_PLAN.md`](../CCSWITCH_MECHANISM_AUDIT_AND_REBUILD_PLAN.md)
> 取代。当前唯一机制合同是后者。
>
> **本文中以下结论已被推翻，请勿据此编码：**
>
> | 本文档的旧结论 | 当前实际架构 |
> |---|---|
> | 必须保持 `model_provider = "openai"`，不得创建 `[model_providers.*]` | 集成层**会**写入 `model_provider = "omnibridge"` 与 `[model_providers.omnibridge]`（见 `crates/integration/src/lib.rs`） |
> | 不得读取、修改或复制 `auth.json` / OAuth 令牌 | 账号管理功能会读取 `auth.json`，并在用户主动切换账号时备份并改写它（见 `crates/manager/src/account.rs`）；令牌现存储于系统钥匙串 |
> | 集成只改 `model_catalog_json` 一个字段 | 集成额外管理 `model_provider` 与 `[model_providers.omnibridge]` |
> | 以 patched upstream Codex Core 实现每轮路由 | 已放弃该路线；改为 stock Codex + 单一 OmniBridge provider |
>
> 本文档中的**祈使句（"必须"、"不得"、"停止条件"、"验收门槛"）一律失效**，仅作为
> 了解历史决策过程的背景资料保留。当前的问题清单与修复计划见
> [`docs/CODE_AUDIT_AND_FIX_PLAN.md`](../CODE_AUDIT_AND_FIX_PLAN.md)。
>
> 原始文档正文自下一行起保留，未做删改。
